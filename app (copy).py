...
def explicit_recs_ratings():
    ...
    if request.method == 'POST':
        ...
        sys.stdout.flush()
        # print(interactions_explicit)
        # for i in interactions_explicit:
        #     print(interactions_explicit[i][0])
        # if (user_id not in interactions_explicit[:][0]):
        # weights_explicit_arr = np.concatenate((weights_explicit_arr, [weights_array]))
        # interactions_explicit_arr = np.concatenate((interactions_explicit_arr, [interactions_array]))
        # ratings = get_ratings()
        # id_users_books = StoreValue()
