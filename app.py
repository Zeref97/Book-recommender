...
def login():
    ...
    if request.method == 'POST':
        ...
        mycursor.execute("USE book_recommender")
        # mycursor.execute("SELECT * FROM users where user_name = 'aaa'")
        # print(username)
        sql = """SELECT * FROM users WHERE user_name = %s"""

        mycursor.execute(sql, (username, ))
        result = mycursor.fetchall()
        # result = mycursor.execute("SELECT * FROM users WHERE user_name = " + username )
        print("=====================>", result)
        if result:
            user_id_raw = result[0][0]
            if user_id_raw not in map_user_book._user_id:
                map_user_book._user_id.append(user_id_raw)
...
def explicit_recs_ratings():
    ...
    if request.method == 'POST':
        ...
        # book_id_raw = np.array(map_user_book._book_id)[pos].map(item_id_rev_map)
        # for k, v in item_id_map.items():
        #     if k==
        # print(len(user_id_map.keys()))
        book_id_key = []
        for k in item_id_map.keys():
            book_id_key.append(k)
        book_id_raw = np.array(book_id_key)[pos]
        # book_id_raw = np.array(item_id_map.keys())[pos]
